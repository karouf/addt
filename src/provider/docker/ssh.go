...
func (p *DockerProvider) HandleSSHForwarding(forwardKeys bool, forwardMode, sshDir, username string, allowedKeys []string) []string {
	if !forwardKeys {
		return nil
	}
...
func (p *DockerProvider) mountSafeSSHFiles(sshDir, username string) []string {
	var args []string

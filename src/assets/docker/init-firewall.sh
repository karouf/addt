...
# Default allowed domains for DClaude
...
if [ "${DCLAUDE_FIREWALL_MODE}" = "strict" ] || [ "${DCLAUDE_FIREWALL_MODE}" = "enabled" ]; then
...
elif [ "${DCLAUDE_FIREWALL_MODE}" = "permissive" ]; then
...
echo "Firewall: Mode: ${DCLAUDE_FIREWALL_MODE:-strict}"
